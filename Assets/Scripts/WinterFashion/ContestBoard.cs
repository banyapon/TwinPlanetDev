...
public class ContestBoard : MonoBehaviour
{
    ...
    IEnumerator GetContestList()
    {
        ...
        userCount = _userCount.Count;
        userID = new string[userCount];
        userCode = new string[userCount];

        for (int i = 0; i < userCount; i++) //IMPORT ALL USER ID & USER CODE
        {           
            userID[i] = _userCount[i]["docno"].ToString().Replace('"', ' ').Trim();
            userCode[i] = _userCount[i]["uscode"].ToString().Replace('"', ' ').Trim();
        }
